...
class Farm {
    ...
    async Inventory(): Promise<Inventory> {
        const nfts = await Opensea.ListNFTs(this.address);
        const farmInventory = [];
        const promises = [];
        for (let idx = 0; idx < nfts.length; idx++) {
            const nft = nfts[idx];
            if (nft.contract.trim().toLowerCase() == inventoryContractAddress.trim().toLowerCase()) {
                const item = {
                    nft: nft,
                    quantity: 0
                };
                promises.push(this.Amount(nft.identifier).then((amount) => {
                    item.quantity = Number(amount);
                }));
                farmInventory.push(item);
            }
        }
        await Promise.all(promises);
        return new Inventory(farmInventory);
    }

    /**
     * The amount of the given asset you have stored
     * @param id The asset's ID, check opensea for this number.
     * @param precision The number of digits places to return.
     */
    async Amount(id: string, precision: number = 10): Promise<number> {
        const amount = await inventory.methods.balanceOf(this.address, id).call();
        const multiplier = 10n**BigInt(precision);
        if (amount > 10n**15n) {
            const result = amount * multiplier / 10n**18n;
            return Number(result) / Number(multiplier);
        } else {
